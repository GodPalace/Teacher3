        ...
        try {
            switch (command) {
                ...
                case GET_FILE_TYPE -> {
                    ...
                    File file = new File(path);
                    if (file.exists()) {
                        responseBytes.writeInt((path.endsWith(File.separator)? RemoteFileType.ROOT : (file.isFile()? RemoteFileType.FILE : RemoteFileType.DIRECTORY)).getType());
                    } else {
                ...
                case LIST_FILES -> {
                    ...
                    if (file.exists()) {
                        if (file.isDirectory()) {
                            boolean isRoot = path.equals(File.separator);

                            File[] files = (isRoot? File.listRoots() : file.listFiles());
                            ...
                            for (int i = 0; i < count; i++) {
                                RemoteFileType type = (isRoot? RemoteFileType.ROOT :
                                        (files[i].isFile()? RemoteFileType.FILE : RemoteFileType.DIRECTORY));
                                new RemoteFile(files[i].getAbsolutePath(), type).writeToStream(objOut);
                            }
